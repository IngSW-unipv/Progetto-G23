...
public class AdminStoreView extends AView {
	...
	public AdminStoreView(HashMap<Merchandising, Integer> merch, ArrayList<AcquistoStore> acquisti, Dimension dim) {
		merch_n = merch.size();
	...
	public Collection<StoreButton> getModifyBtns() {
		return itemPanels.values().stream().map(panel -> panel.getModifyBtn()).collect(Collectors.toList());
	}
