...
public class PostoView extends AView {
	...
	public PostoView(Dimension dim, Stadio stadio, int n_settore, int n_anello, int n_blocco) {
		...
		gruppo = new ArrayList<>(Stadio.POSTI_PER_BLOCCO);
		posto = new ArrayList<>(Stadio.POSTI_PER_BLOCCO);
		nomeposto = new ArrayList<>(Stadio.POSTI_PER_BLOCCO);
		...
		img = new ImageIcon(img.getImage().getScaledInstance((dim.width) / 10, (dim.height - 100) / 5,
		...
		for (int i = 0; i < Stadio.POSTI_PER_BLOCCO; i++) {
			...
			gruppo.get(n_posto).setPreferredSize(new Dimension((dim.width) / 10, (dim.height - 45) / 5));
		...
		blocco = new JPanel();
		blocco.setPreferredSize(new Dimension(dim.width, (dim.height - 45)));
