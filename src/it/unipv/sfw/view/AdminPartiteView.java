...
public class AdminPartiteView extends AView {
	...
	public AdminPartiteView(Partita[] par, Dimension dim) {
		...
		infoPartita = new ArrayList<>();
		immagini = new ArrayList<>();

		Calendar d = new GregorianCalendar();
		ArrayList<JPanel> bottone = new ArrayList<>();

		for (int j = 0; j < righe; j++) {
			d = par[j].getCalendarDate();
			partite.add(new JLabel(
					"<html> " + par[j].getOspiti() + "<br><br>" + par[j].getDataPerPartita() + "<br><br></html>"));
			i.add(new JMenuBar());
			info.add(new InfoMenu("informazioni", j));
			infoPartita.add(new JLabel(
					"<html>Posti occupati: <br><br>Clienti presenti: <br><br>Abbonati presenti: &nbsp &nbsp<br><br>Totale ricavi: </html> "));
	...
	/**
	 * Metodo chaimato quando si vuoel chiudere il popup ddelle infromazioni sulla
	 * partita.
	 *
	 * @param stato
	 * @param code
	...
	public void OpenInfoPartita(boolean stato, int code, int postioccupati, int clientipresenti, int abbonatipresenti,
			double ricavi) {
		info.get(code).setPopupMenuVisible(stato);
		infoPartita.get(code)
				.setText("<html>Posti occupati: " + postioccupati + "<br><br>Clienti presenti: " + clientipresenti
						+ "<br><br>Abbonati presenti: " + abbonatipresenti + "<br><br>Totale ricavi: " + ricavi
						+ "</html>");
	}
