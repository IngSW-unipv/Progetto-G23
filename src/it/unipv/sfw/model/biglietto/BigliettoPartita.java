...
/**
 * Classe che rappresenta il biglietto della partita acquistabile dal
...
public class BigliettoPartita extends Biglietto {
	...
	public boolean setAnno(int anno) {
		...
		if (anno >= calendar.get(Calendar.YEAR))
			dataDisponibilita.set(Calendar.YEAR, anno);
		else
			return false;
		return true;
	}
	...
	public boolean setGiorno(int giorno) {
		if (giorno > 0 && giorno < 32)
			dataDisponibilita.set(Calendar.DATE, giorno);
		else
			return false;
		return true;
	}
	...
	public boolean setMese(int mese) {
		if (mese >= 0 && mese < 12)
			dataDisponibilita.set(Calendar.MONTH, mese);
		else
			return false;
		return true;
	}
