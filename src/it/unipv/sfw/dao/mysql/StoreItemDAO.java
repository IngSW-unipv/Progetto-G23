...
public class StoreItemDAO implements IStoreItemDAO {
	...
	public HashMap<Merchandising, Integer> selectAll() {
		...
		try (DBConnection db = new DBConnection(SCHEMA)) {
			...
			while (rs1.next()) {
				int q = rs1.getInt(4);
				Merchandising c = new Merchandising(rs1.getString(2), rs1.getDouble(3), rs1.getInt(1),
						rs1.getString(5));
				result.put(c, q);
			}
	...
	public AbstractMap.SimpleEntry<Merchandising, Integer> selectById(Merchandising merch) {
		...
		try (DBConnection db = new DBConnection(SCHEMA)) {
			...
			while (rs1.next()) {
				int q = rs1.getInt(4);
				Merchandising m = new Merchandising(rs1.getString(2), rs1.getDouble(3), rs1.getInt(1),
						rs1.getString(5));
	...
	public boolean updateItem(Pair<Merchandising, Integer> merch) {
		...
		try (DBConnection db = new DBConnection(SCHEMA)) {
			...
			String query = "UPDATE " + SCHEMA
					+ " SET NOME=?, SET DESCRIZIONE=?, SET QUANTITA_RIMANENTE=?, SET PREZZO=? WHERE ID=?";
			st1 = conn.prepareStatement(query);
