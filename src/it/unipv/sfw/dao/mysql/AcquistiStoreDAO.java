	...
	public ArrayList<AcquistoStore> selectAllWithPrice() {
		...
		try (DBConnection db = new DBConnection(SCHEMA)) {
			...
			String query = "SELECT ITEM, BUYER, QUANTITA, B.NOME, PREZZO, QUANTITA_RIMANENTE, DESCRIZIONE,"
					+ " C.NOME, COGNOME, PASS, NASCITA " + "FROM " + SCHEMA
					+ " A JOIN STORE_ITEMS B on A.ITEM=B.ID JOIN UTENTI C on A.BUYER=C.EMAIL";

			rs1 = st1.executeQuery(query);

			while (rs1.next()) {

				Merchandising m = new Merchandising(rs1.getString(4), rs1.getDouble(5), rs1.getInt(1),
						rs1.getString(7));
